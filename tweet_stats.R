...
## @knitr splot
ggplot(sentiment, aes(date)) + geom_line(aes(y=positive_sentiment), colour="#18699c",size=1.5) +  geom_line(aes(y=negative_sentiment), colour="#a32e3a",size=1.5) + ylab("Count") + xlab("Time")  + opts(title="Sentiment of tweets under #ESA2012 hashtag (blue = positive, red = negative)")

## @knitr totalplot
ggplot(sentiment, aes(date)) + opts(title="Number of tweets under #ESA2012 hashtag") +   geom_line(aes(y=mentions), colour="#474747",size=1.5) + ylab("Count") + xlab("Time")
